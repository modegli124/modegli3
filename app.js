...
function openTool(id){const t=tools.find(x=>x.id===id);panel.hidden=false;content.innerHTML=`<div class="tool-box"><p class="eyebrow">MODEGLI TOOLS</p>${t.make()}</div>`;window.scrollTo(0,0);if(id==="qr")loadQR();if(id==="compress")setupCompress();if(id==="words")setupWords();if(id==="units")setupUnits();if(id==="calc")setupCalc();if(id==="ai")setupAI()}
